...
namespace Shadow
{
	DescriptorSetAllocator::DescriptorSetAllocator(const std::array<DescriptorSetLayout, 4>& layouts)
	{
		...
		VK_CHECK_RESULT(vkCreateDescriptorPool(VulkanContext::getDevice()->getVkDevice(), &poolinfo, nullptr, &m_descriptorPool));
	}

	DescriptorSetAllocator::~DescriptorSetAllocator()
	{
		vkDeviceWaitIdle(VulkanContext::getDevice()->getVkDevice());
		vkDestroyDescriptorPool(VulkanContext::getDevice()->getVkDevice(), m_descriptorPool, nullptr);
	}
	...
	void DescriptorSetAllocator::allocateDescriptorSets(std::array<VkDescriptorSet, 4>& sets, const std::array<VkDescriptorSetLayout, 4>& layouts)
	{
		...
		VK_CHECK_RESULT(vkAllocateDescriptorSets(VulkanContext::getDevice()->getVkDevice(), &allocInfo, sets.data()));
	}
}